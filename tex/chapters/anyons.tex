...
This chapter investigates methods that define tensor networks on physical systems that
...
anyons are their universality for quantum computation. In \Cref{anyons:sec:topological_quantum_computation} we provide an
interpretation of how both normal braiding circuits and a variation in terms of
\emph{weaves} can be simulated by the tensor network theory.
...
Generally when speaking about anyons, we group them into \emph{theories}. A anyon theory is equipped with a (finite) set of charges $a,b,c,\ldots$ including a \emph{vacuum} charge $I$. Additionally we are provided with a \emph{fusion rule} $a \times b = \sum_{c}^{} N_{ab}^c c$ where $N_{ab}^c$ denotes the multiplicity. Since it can be shown (should I?) that we cannot classify the vector space of two anyons $a,b$ by itself, we need to parametrize this space over its \emph{fusion sectors}, i.e. the outcomes $c$ of the fusion rule. Let $V^{ab}_c$ be the space of 2 anyons of charges $a,b$ fusing to $c$. Then the whole space $V^{ab}$ splits accordingly:
\[
