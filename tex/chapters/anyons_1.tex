...
\[
  \mathcal{H} \overset{\pi}{\longrightarrow} \uConf_n(\Sigma),
\]
i.e. to each point $[x] \in \uConf_n(\Sigma)$ there is an associated Hilbert
space $\mathcal{H}_{x}$. Furthermore, a path through the configuration space
$[\gamma] : x_0 \to x_1 \in \Br_n(\Sigma) = \pi_1(\uConf_n(\Sigma))$ is lifted to some unitary
representation
\[
