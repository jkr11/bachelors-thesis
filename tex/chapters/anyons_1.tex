...
physical state, so we denote the full space of our system by
\[
    \Sigma^n / S_n.
\] 
...
To make the intuition of non-contractibility in ??? formal, we need to introduce
some tools from algebraic geometry. Specifically, we introduce the
\emph{fundamental group}. The fundamental group collects all the loops at a
basepoint $x_0$ in the space $\Sigma$ up to their homotopy equivalences. If $D =
2$ it is known that
\[
    \pi_1(\uConf_n (\Sigma)) = \Br_n(\Sigma) \quad \text{ and } \quad \pi_1(\oConf{n} (\Sigma)) = \PBr_n(\Sigma)
\]
however if $D >= 3$ we know that these coincide with the symmetric group
instead\todo{rephrase a lot!}. In fact there is an exact sequence
\[
    1 
    \longrightarrow 
    \mathrm{PBr}_n(\Sigma) 
