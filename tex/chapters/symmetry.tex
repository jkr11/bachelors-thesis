...
In the following, let $G$ be a compact group and $V,W$ be finite dimensional vector spaces.
...
\begin{definition}[???]
  ...
  \]
  where we call $N^{ab}_c$ the \emph{fusion multiplicty} in $G$. If $N_{ab}^c \leq 1$ for all $a,b,c$, we say that the representation is \emph{multiplicity-free}.  
\end{definition}
