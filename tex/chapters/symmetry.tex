...
\begin{definition}[Representation]
  A representation of a group $G$ over a vector space $V$ is a group homomorphism
  \[
    \rho : G \to \GL V, \quad \rho(g)\rho(h) = \rho(g \cdot h), \quad \rho(e) = id_V
  \]
\end{definition}
We can state this equvalently as the representation under the group action of $G$ on $V$:
\begin{definition}[Representation in action form]
  Given a group $G$ and a vector space $V$, a representation is a map
  \[
...
\begin{definition}[Intertwiner]
  A linear map $A : V \to W$ is an intertwiner, if under representations $\rho_V : G \to \GL(V)$ and $\rho_W \to \GL W$ it holds that
  \[
    T \circ \rho_V(g) = \rho_W (g) \circ T \quad \forall g \in G
  \]
  we denote the space of all intertwiners as $\Hom_G(V,W)$.
\end{definition}
...
\begin{definition}[Tensor product representation]
  Given reps $\rho_V$ and $\rho_W$, the \emph{tensor product representation}
  \[
    \rho_V \otimes \rho_W : G \to  \GL {V \otimes W}
  \]
\end{definition}

\begin{definition}[Direct sum representation]
  As above, the direct sum representation is a homomorphism 
  \[
    \rho_V \oplus \rho_W : G \to \GL {V \oplus W},
  \]
\end{definition}
...
\begin{definition}[Isotypic / Semisimple decomposition]
  ...
  \]
  by grouping isomorphic simples.
\end{definition}

\begin{definition}[???]
  Let $R_a$ and $R_b$ be two irreducible representation (spaces). Their tensor product is again semisimple and there is a decomposition
  \[
    R_{a} \otimes R_{b} \cong \bigoplus_{c \in I} R_c^{\oplus N^{ab}_c}   
  \]
  where we call $N^{ab}_c$ the \emph{fusion multiplicty} in $G$. 
\end{definition}

\begin{example}[Clebsch-Gordan series]
  Let $G = \SU{2}$ and let $V_i$ and $V_j$ be two spin irreps of charge $i$ and $j$. These combine to
  \[
    (\pi_{V_i \otimes V_{j}}, V_i \otimes V_j) = \bigoplus_{k \in \left\vert i - j \right\vert}^{i + j} (\pi_{V_k}, V_k) 
  \]
\end{example}
