...
On the operators between tensor products, i.e. $T \in \End(R_a \otimes R_b)$ this yields a formula
\[
...
diagonalization}\cite[Thm~1.3.12]{Horn_Johnson_2012}, as shown in \Cref{fig:diagonal_hamiltonians}. Notably these presentation
induce a \emph{block-sparse} structure on the respective operators. 
...
Now we can analize a example with multiplicites: Consider the same hamiltonian but acting on the ground state $V_{\frac{1}{2}}^{\otimes 3}$ with the hamiltonina $H_2 \otimes \id_{V_{\frac{1}{2}}}$. First note that there are two ways we can obtain this fusion result, and we choose the left bracketing $(\frac{1}{2} \otimes \frac{1}{2}) \otimes \frac{1}{2} = (V_0 \oplus V_1) \otimes V_\frac{1}{2} = V_\frac{1}{2} \oplus V_\frac{1}{2} \oplus V_{\frac{3}{2}}$. By this decomposition we should expect a operator of the form
\[
  H \in Mat_{2\times 2} \otimes \id_{\frac{1}{2}} \oplus \C \otimes \id_{\frac{3}{2}}
\]

Lets do the visual analysis here first:
\begin{equation}
  \begin{tikzpicture}[
    baseline=(current bounding box.center),
    scale=1.0,
  ...
  \end{tikzpicture}
\end{equation}
which reveals the action to be 
\[
  S_1 \cdot S_2 \otimes id_{\frac{V_1}{2}} = \begin{pmatrix}
    -\frac{3}{4} & 0  \\
    0 & \frac{1}{4}  \\
  \end{pmatrix} \otimes \id_{\frac{1}{2}} \oplus \frac{1}{4} \otimes \id_{\frac{3}{2}}
\]
