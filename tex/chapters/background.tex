...
Generally, we want to regard a tensor as a multilinear map equipped with legs, where each leg holds a vector space. But we also want to be able to regard tensors as operators from a \emph{domain} vector space to a \emph{codomain} vector space, such as Hamiltonians. So we develop the theory after \textcite{Penrose1971Applications}.
...
Given \emph{finite} dimensional vector spaces $V$ and $W$, we have the identity that $V \otimes W^\ast \equiv Hom(W,V)$. This is called the \emph{hom-tensor duality}. (TODO: is this canonical? What about the other way around?)
