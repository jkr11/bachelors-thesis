...
already met in the discussion on representations and anyons.
\Cref{cat:sub:categoricalQuantum} provides a short review of how familiar structures such
...
\begin{definition}
  A category $\CC$ is called \green{\emph{semisimple}} if every object can be written as a direct sum of simple objects. TODO: simple. 
\end{definition}

Recall that for semisimple categories, there is an isomorphism $\Phi: \bigoplus_{c,\mu}
R_c \to R_a \otimes R_b$. Then we can build projection and injection maps in dependence of
the tensor product by the following diagram:
...
Let $k$ be a field, and let $\mathcal{C}$ and $\mathcal{D}$ be two $k$-linear, finite
abelian categories. The \textbf{Deligne tensor product}, denoted by $\mathcal{C} \boxtimes
\mathcal{D}$, is a $k$-linear finite abelian category equipped with a $k$-bilinear functor
...
\begin{definition}[The Category $\Rep_k(G)$]
  ...
  \begin{itemize}
    \item \textbf{Objects:} The objects $\Rep_k(G)$ are pairs $(V, \rho_V)$, where $V$ is
    a $k$-vector space and $\rho_V : G  \to \mathrm{GL}(V)$.  
    \item \textbf{Morphisms:} For any two objects $(V, \rho_V)$ and $(W, \rho_W)$ in
    $\Rep_k(G)$, a morphism from $V$ to $W$ is a $k$-linear transformation $f: V \to W$
    that is $G$-invariant (also called an \textit{intertwiner}). That is, for every
    element $g \in G$, the following diagram commutes:
    \[
    \begin{tikzcd}
