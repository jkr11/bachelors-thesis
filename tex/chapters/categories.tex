...
The general idea of a category is that it admits a sort of \emph{interface}. As noticed, there are many similarities in data and structure between the entities we already dealt with, i.e. the tensor product of vector space, the tensor product of representations, of Hilbert spaces etc.... We can view all these concepts as a category with additional constraints added.

\begin{definition}[Category]\label{cat:def:category}
  A \emph{\red {category}} $\mathcal{C}$ consists of
  \begin{itemize}
...
TODO: state no cloning after \cite{abramskyNoCloningCategoricalQuantum2012}
...
A \emph{braided monoidal category} is a monoidal category $(\mathcal{C}, \otimes, I, \alpha, \lambda, \rho)$ equipped with an additional structural natural isomorphism, the \emph{braiding}
\[
...
A braided monoidal category is \emph{symmetric} if $\beta_{B,A} \circ \beta_{A,B} = \mathrm{id}_{A \otimes B}$ for all $A, B$.
\end{definition}

%\begin{remark}
%Symmetric monoidal categories are exactly the setting in which the categorical no-cloning obstruction is vacuous in one direction and sharp in the other: $\mathrm{Set}$ (cartesian, hence symmetric) admits natural copying, while $\mathrm{FdHilb}$, though symmetric ($\beta$ is the usual swap $v \otimes w \mapsto w \otimes v$), is \emph{not} cartesian, and it is exactly this failure — not the presence or absence of $\beta$ itself — that obstructs cloning. For anyon models, $\beta_{A,B} \neq \beta_{B,A}^{-1}$ in general (nontrivial braiding, not mere permutation), which is precisely the extra data making a modular tensor category non-symmetric and physically encoding anyonic exchange statistics.
%\end{remark}
...
\subsection{Decomposition of semisimple Categories}
...
\begin{corollary}[Operators are block matrices \cite{kongInvitationTopologicalOrders2022b}] % TODO figure out why i cant cie theorems in thm header.
...
and
\begin{equation}
  \sum_{c,\mu}^{} \iota_{c,\mu}^{ab} \pi^{c,\mu}_{ab} = \Phi \sum_{c,\mu}^{} \iota_{c,\mu}\pi_{c,\mu} \Phi^{-1} = \Phi \id_{\bigoplus R_{c}} \Phi^{-1} = \id_{R_a \otimes R_b}  
\end{equation}
