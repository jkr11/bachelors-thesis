...
\begin{definition}[Category]
  \label{def:category}
  A category $\mathrm{C} $  is build by a collection of of objects, which we usually denote by $obj(\mathrm{C} )$, and a set of morphisms $\Hom_C(A,B)$ for each pair $A,B$ of objects, where we refer to $A$ as the domain and $B$ as the codomain. If we want to refer to an object in $\mathrm{C} $, we usually just write $A \in \mathrm{C}$.

  Furthremore, there is a binary operator $\circ : Hom_C(A,B) \times Hom_C(B, C) \to Hom(A,C)$ such that this is associative. Also, we have an identity morphism for each object $A$ denoted by $id_A$ s.t. $id_A \circ f = f$ and $g \circ id_B = g$ for all appropriate morphisms. 
...
\begin{definition}[Monoidal Category]
  ...
  A \emph{monoidal category} is a 5-tuple $(\mathrm{C}, \otimes, I, \alpha, \lambda)$ where ...
\end{definition}
...
\begin{definition}[Matrix Product State]
  ...
  \]
  where $A^{[i]}_{i_{k}} \in \mathbb{C}^{D_i \times D_{i} }$.
\end{definition}
...
\begin{lemma}[IDK if this is true.]
  A MPS state $\ket{\psi[A]}$ is symmetric under $G$ iff. for every tensor $A^{[i]}$ it holds that
  \[
    U(g)A^{[i]}U(g)^{\dagger} = A^{[i]} 
  \]
\end{lemma}
...
\begin{theorem}[Fundamental Theorem of MPS]
  ...
  Proof \cite{ciracMatrixProductDensity2017},\cite{vercleyenMathematicalStructureTensor2018}.
\end{theorem}



\subsection{Review of Anyonic Tensor Networks.}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.8\textwidth]{../experiments/plots/xxz_hamiltonian.pdf}
  \caption{\texttt{XXZ} Hamiltonian in both bases.}
...
Now, we can realize about this hamiltonian is that there is a symmetry with the representations of $U(1)$. Lets review this shortly. A representation of $U(1)$ is a map
\[
...
\[
  [H,S^z_{\mathrm{tot}}]=0.
\]
Recall that this last condition entails a block diagonal basis for $H$, as $H$ and $S^z$ are simultaneously diagonalizable.
...
Here we review the golden chain hamiltonian as introduced by \cite{feiguinInteractingAnyonsTopological2007} and extended in \cite{trebstShortIntroductionFibonacci2008a} as well as generalizations in \cite{gilsAnyonicQuantumSpin2013} and a general account for arbitrary anyon models in \cite{buicanAnyonicChainsTopological2017}. For a treatment in terms of anyonic tensor networks see \cite{ayeniStudiesBraidedNonAbelian2017} and \cite{pfeiferSimulationAnyonsTensor2010} as well as \cite{pfeiferSimulationAnyonsUsing2012}. Furthre generalizations of spin chains using modular categories are given in \cite{eckQuantumLatticeModels2025} (find the others here or what the correct inheritance is).
...
Some sources here: On Lattice Models with Module Categories: \cite{eckQuantumLatticeModels}, \cite{lootensDualitiesOneDimensionalQuantum2023} \cite{lootensDualitiesOneDimensionalQuantum2024} \cite{lootensEntanglementDensityMatrix2025}. Also chapters IV and V of \cite{cuiperHouchesLectureNotes2026}. On the structure see \cite{liuTradingMathematicalPhysical2025}
